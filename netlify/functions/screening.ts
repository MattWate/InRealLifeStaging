...
async function getList(sql: any) {
  const rows = await sql`
    select pp.id, pp.name, pp.slug, pp.city, pp.region, pp.country_code,
      pp.property_types, pp.status, pp.updated_at, op.name as operator_name,
    ...
    left join lateral (
      ...
      order by psa.assessment_number desc limit 1
    ) a on true
...
function calculate(body: Payload) {
  ...
  const propertyTypes = array(body.property_types);
  ...
  const maps:Record<string,Record<string,number>>={
    ...
    average_nightly_rate_band:{under_800:2,'800_to_1500':5,'1500_to_3000':7,'3000_plus':8},
